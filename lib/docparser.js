    ...
    var Lang = Y.Lang,
        ...
        DIGESTERS = {
            ...
            'param': function (tagname, value, target, block) {
                ...
                var type, name, originalName, searchFor, parts, optional, optdefault, parent, multiple, len, result,
                ...
                if (name.indexOf('[') > -1) {
                    match = REGEX_NESTED_PARAM.exec(name);
                    if (match) {
                        name = trim(match[1]);
                        if (name.indexOf('?') > -1) optional = true;
                ...
                if (name.indexOf('.') > -1) {
                    originalName = name;
                    match = name.split('.');
                    parent = match.shift(); // first level parent is always the first.
                    name = match.pop(); // name is always the last.

                    searchFor = function (_target) {
                        Y.each(_target, function (param) {
                            if (param.name === parent) {
                                if (match.length > 0) {
                                    parent = match.shift();
                                    if (!param.props) {
                                        var errorMessage = 'Nested param annotated without a parent:\n\tparam name: ' +
                                            originalName + '\n\tfile name: ' + target.file;
                                        throw errorMessage;
                                    }
                                    searchFor(param.props); // recursively search for nested props
                                }
                                else {
                                    if (!param.props) {
                                        param.props = [];
        ...
        DocParser = function (o) {
            this.digesters = Y.merge(DocParser.DIGESTERS, o.digesters);
            for (var key in o.digesters) DocParser.TAGLIST.push(key);
